...
import numpy as np
import unittest
from toasty import Mass, AvgTemp, PenalizeDensity, SmoothStep, LinearDensityFilter
...
class TestSmoothStep(unittest.TestCase):
    ...
    def test_partials(self):
        ...
        input = self.rand.random(((nx - 1) * (ny - 1)))

        p = om.Problem()
        p.model.add_subsystem("step", SmoothStep(num_x=nx, num_y=ny), promotes=["*"])
        p.setup()

        p.set_val("in", input)

        om_assert.assert_check_partials(p.check_partials(), atol=5e-5, rtol=5e-6)


class TestLinearDensityFilter(unittest.TestCase):
    def setUp(self):
        self.rand = np.random.default_rng(58)
    ...
    def test_partials(self):
        ...
        density = self.rand.random(((nx - 1) * (ny - 1)))

        p = om.Problem()
        p.model.add_subsystem("filter", LinearDensityFilter(num_x=nx, num_y=ny), promotes=["*"])
        p.setup()

        p.set_val("density", density)
